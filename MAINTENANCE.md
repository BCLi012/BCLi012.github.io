...
网站如何修改与维护。站点是什么、有哪些栏目，见 [README.md](README.md)；架构原理见 [ARCHITECTURE.md](ARCHITECTURE.md)。
...
改任何内容都只是改文件，推送到 GitHub 后自动重新构建。不需要 FTP、不需要部署脚本。

**注意**：GitHub Pages 给每个页面约 10 分钟缓存。推送后如果没看到变化，用 `Cmd + Shift + R` 强制刷新，或开无痕窗口确认。
...
`url` 留空的条目不会显示；`url_zh` 可选，用于中文页指向不同地址（如站内路径 `/zh/…`）。

### 换头像

覆盖 `assets/images/profile.jpg` 即可。建议宽度 640px 左右、竖版或正方形。

如果换成横版照片，需要同步改 `assets/css/styles.css` 里 `.profile-photo img` 的 `aspect-ratio`。
...
配色集中在 `assets/css/styles.css` **最顶部**的设计令牌，改了全局生效：
...
**同一信息尽量只写一处**：教育背景、名言、社交链接都存在 `_data/` 里，多个页面共同引用。若要新增这类"会在多处出现"的信息，优先加进 `_data/`，而不是分散写在页面里。
